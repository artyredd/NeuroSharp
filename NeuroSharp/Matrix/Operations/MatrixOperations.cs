...
namespace NeuroSharp
{
    public static class MatrixOperations
    {
            ...
            where T : unmanaged, IComparable<T>, IEquatable<T>
        {
            ...
            // test to see if transposing the matrix first is faster
            IMatrix<T> Transposed = Right.Transpose();
            ...
            for (int LeftRow = 0; LeftRow < Left.Rows; LeftRow++)
            {
                ...
                {
                    Span<T> OtherRow = new(Transposed[RightRow]);
                    ...
                    for (int LeftColumn = 0; LeftColumn < Left.Columns; LeftColumn++)
                    {
                        ...
                        T multiplied = Multiplier(ref Row[LeftColumn], ref OtherRow[LeftColumn]);
        ...
        public static Span<T> MultiplyMatricesOld<T>(IMatrix<T> Left, IMatrix<T> Right, TwoElementOperation<T> Multiplier, TwoElementOperation<T> Adder)
            where T : unmanaged, IComparable<T>, IEquatable<T>
        {
            // to multiply two matrices we calc the dot product of the first row of the left with the first column of the right
            // we can either tranpose the whole matrix or use referenced mapped indices
